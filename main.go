...
func walkFiles(config Config) (<-chan FileMetadata, <-chan error) {
	...
	go func() {
		...
		// Only report fatal errors that prevent the entire process from continuing
		if err != nil && err != filepath.SkipDir {
			log.Printf("Error during walk: %v", err)
			// Don't send to errChan unless it's truly fatal
		}
	}()

	return metadataChan, errChan
}

// writeParquet writes metadata to a Parquet file
func writeParquet(config Config, metadataChan <-chan FileMetadata, errChan <-chan error) error {
	...
	case err := <-errChan:
		if err != nil {
			return fmt.Errorf("error during file walk: %v", err)
		}
	default:
	}
...
func main() {
	...
	metadataChan, errChan := walkFiles(config)
	if err := writeParquet(config, metadataChan, errChan); err != nil {
		log.Fatal(err)
	}
}